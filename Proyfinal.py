...
frio = load_model('frio.pkl')
caliente = load_model('caliente.pkl')

def predictor(clf, df): 
    
    predictions_data = predict_model(estimator = clf, data = df) 
...
Autmaxv = st.sidebar.slider(label = 'Autonomía a máxima velocidad (kms)', min_value =30, max_value = 560 , value = 30, step = 10)
...
tab_plots= tabs[0]
with tab_plots:
    st.write('En nuestro Dataset inicial contamos con 27 columnas y ningún dato nulo, pero como veremos a continuación, hemos tenido que preprocesar los datos.')
...
with tab_plots:
    ...
    p = open("auton.html")
    components.html(p.read(),height=500,width=1300) 
tab_plots= tabs[3]
with tab_plots:
...
tab_plots= tabs[4]
with tab_plots:
    st.markdown("<h3 style='text-align: center; color: black;'>¿Cuantos kms de autonomía tendrá el vehículo en un clima frío?</h1>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
...
tab_plots= tabs[5]
with tab_plots:
    st.markdown("<h3 style='text-align: center; color: black;'>¿Cuantos kms de autonomía tendrá el vehículo en un clima cálido?</h1>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        st.subheader('Predicción')
        st.write('Seleccione las carácterísticas de un vehículo con el panel desplegable de la izquierda.')
